...
.EXAMPLE
    Get-WWConfiguration -filename "C:\path\to\config.xml"
#>
function Get-WWConfiguration {
    [CmdletBinding()]
    param(
    ...
    process {
        ...
        try {
            ...
            Import-WWWIMInformation -IndexNumber $WPFSourceWimIndexTextBox.text -SkipUserConfirmation

            if ($WPFJSONEnableCheckBox.IsChecked -eq $true) {
                Convert-WWJSONToObject -file $WPFJSONTextBox.text
            }

            # Handle ConfigMgr specific settings
            if ($WPFCMCBImageType.SelectedItem -eq 'Update Existing Image') {
                Get-WWImageInformation -PackID $settings.CMPackageID
            }
