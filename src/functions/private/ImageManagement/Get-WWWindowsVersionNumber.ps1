...
.NOTES
    Name:        Get-WWWindowsVersionNumber.ps1
...
.EXAMPLE
    Get-WWWindowsVersionNumber
#>
function Get-WWWindowsVersionNumber {
    [CmdletBinding()]
    param(
    ...
    process {
        ...
        If ($WPFSourceWimVerTextBox.text -like '10.0.19041.*') {
            ...
            If ($IsMountPoint -eq $False) {
                ...
                Write-WimWitchLog -data 'Prompting user for Win10 version confirmation...' -class Information
                ...
                $buildnum = $temp
                Write-WimWitchLog -data "User selected $buildnum" -class Information
