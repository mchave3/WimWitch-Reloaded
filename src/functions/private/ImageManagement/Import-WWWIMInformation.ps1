...
.NOTES
    Name:        Import-WWWIMInformation.ps1
...
.EXAMPLE
    Import-WWWIMInformation -IndexNumber 1
#>
function Import-WWWIMInformation {
    [CmdletBinding()]
    param(
    ...
    process {
        Write-WimWitchLog -Data 'Importing Source WIM Info' -Class Information
        try {
            ...
            Write-WimWitchLog -data 'The WIM file selected may be borked. Try a different one' -Class Warning
            return
        }
        ...
        if ($SkipUserConfirmation -eq $False) {
            $WPFSourceWimTBVersionNum.text = Get-WWWindowsVersionNumber
        }
    }
}
