...
function Get-WWMicrosoftUpdateItem {
    ...
    process {
        ...
        if ($OptionalUpdateCheck -eq '0') {
            ...
            #Write-WimWitchLog -data "This update appears to be optional. Skipping..." -Class Warning
            #return
            if ($WPFUpdatesCBEnableOptional.IsChecked -eq $True) {
                Write-WimWitchLog -data 'This update appears to be optional. Downloading...' -Class Information
            }
            else {
                Write-WimWitchLog -data 'This update appears to be optional, but are not enabled for download. Skipping...' `
                -Class Information
                return
            }
            #Write-WimWitchLog -data "This update appears to be optional. Downloading..." -Class Information
        ...
        if ($UpdateName -like '*Windows 10*') {
            ...
            #else{
            #Write-WimWitchLog -data "Dynamic updates have not been selected for downloading. Skipping..." -Class Information
            #return
            #}
        }
                ...
                foreach ($UpdateItemContentID in $UpdateItemContentIDs) {
                    ...
                    if ($null -ne $UpdateItemContent) {
                        ...
                        if ((Test-Path -Path $variable) -eq $false) {
                            Write-WimWitchLog -Data "Creating folder $variable" -Class Information
                            New-Item -Path $variable -ItemType Directory | Out-Null
                            Write-WimWitchLog -data 'Created folder' -Class Information
                        } else {
                            $testpath = $variable + '\' + $PSObject.FileName

                            if ((Test-Path -Path $testpath) -eq $true) {
                                Write-WimWitchLog -Data 'Update already exists. Skipping the download...' -Class Information
                                return
                            }
                        }

                        try {
                            Write-WimWitchLog -Data "Downloading update item content from: $($PSObject.SourceURL)" -Class Information
                            ...
                            Write-WimWitchLog -Data "Download completed successfully, renamed file to: $($PSObject.FileName)" -Class Information
                            $ReturnValue = 0
                        } catch [System.Exception] {
                            Write-WimWitchLog -data "Unable to download update item content. Error message: $($_.Exception.Message)" -Class Error
                            $ReturnValue = 1
                        }
                    } else {
                        Write-WimWitchLog -data "Unable to determine update content instance for CI_ID: $($UpdateItemContentID.ContentID)" -Class Error
                        $ReturnValue = 1
                    }
                }
            } else {
                Write-WimWitchLog -Data "Unable to determine ContentID instance for CI_ID: $($UpdateItem.CI_ID)" -Class Error
                $ReturnValue = 1
            }
        } else {
            Write-WimWitchLog -data "Unable to locate update item from SMS Provider for update type: $($UpdateType)" -Class Error
            $ReturnValue = 2
        }
