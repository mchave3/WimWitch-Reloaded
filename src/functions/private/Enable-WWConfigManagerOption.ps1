...
.NOTES
    Name:        Enable-WWConfigManagerOption.ps1
...
#>
function Enable-WWConfigManagerOption {
    [CmdletBinding()]
    param(
    ...
    process {
        ...
        if ($WPFCMCBImageType.SelectedItem -eq 'New Image') {
            ...
            Write-WimWitchLog -data 'ConfigMgr feature enabled. New Image selected' -class Information
            #    Write-WimWitchLog -data $WPFCMTBSitecode.text -class Information
            #    Write-WimWitchLog -data $WPFCMTBSiteServer.text -class Information
        }
        ...
        if ($WPFCMCBImageType.SelectedItem -eq 'Update Existing Image') {
            ...
            #  $WPFCMTBSiteServer.text = $MEMCMsiteinfo.'Site Server'
            #  $WPFCMTBSitecode.text = $MEMCMsiteinfo.'Site Code'
            Write-WimWitchLog -data 'ConfigMgr feature enabled. Update an existing image selected' -class Information
            #   Write-WimWitchLog -data $WPFCMTBSitecode.text -class Information
            #   Write-WimWitchLog -data $WPFCMTBSiteServer.text -class Information
        }
        ...
        if ($WPFCMCBImageType.SelectedItem -eq 'Disabled') {
            ...
            Write-WimWitchLog -data 'ConfigMgr feature disabled' -class Information
