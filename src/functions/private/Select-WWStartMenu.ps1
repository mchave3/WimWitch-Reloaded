    ...
    process {
        ...
        if ($OS -eq 'Windows 11') {
            if ($Sourcexml.FileName -notlike '*.json') {
                Write-WimWitchLog -Data 'A JSON file not selected. Please select a valid file to continue.' -Class Warning
                return
            }
        }
