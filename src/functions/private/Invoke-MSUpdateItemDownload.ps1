...
.EXAMPLE
    Invoke-MSUpdateItemDownload -UpdateName 'KB1234567'
#>
function Invoke-MSUpdateItemDownload {
    ...
    process {
        ...
        if ($OptionalUpdateCheck -eq '0') {
            ...
            #Write-WWLog -data "This update appears to be optional. Skipping..." -Class Warning
            #return
            if ($WPFUpdatesCBEnableOptional.IsChecked -eq $True) {
                Write-WWLog -data 'This update appears to be optional. Downloading...' -Class Information
            }
            else {
                Write-WWLog -data 'This update appears to be optional, but are not enabled for download. Skipping...' `
                -Class Information
                return
            }
            #Write-WWLog -data "This update appears to be optional. Downloading..." -Class Information
        ...
        if ($UpdateName -like '*Windows 10*') {
            ...
            #else{
            #Write-WWLog -data "Dynamic updates have not been selected for downloading. Skipping..." -Class Information
            #return
            #}
        }
                ...
                foreach ($UpdateItemContentID in $UpdateItemContentIDs) {
                    ...
                    if ($null -ne $UpdateItemContent) {
                        ...
                        if ((Test-Path -Path $variable) -eq $false) {
                            Write-WWLog -Data "Creating folder $variable" -Class Information
                            New-Item -Path $variable -ItemType Directory | Out-Null
                            Write-WWLog -data 'Created folder' -Class Information
                        } else {
                            $testpath = $variable + '\' + $PSObject.FileName

                            if ((Test-Path -Path $testpath) -eq $true) {
                                Write-WWLog -Data 'Update already exists. Skipping the download...' -Class Information
                                return
                            }
                        }

                        try {
                            Write-WWLog -Data "Downloading update item content from: $($PSObject.SourceURL)" -Class Information
                            ...
                            Write-WWLog -Data "Download completed successfully, renamed file to: $($PSObject.FileName)" -Class Information
                            $ReturnValue = 0
                        } catch [System.Exception] {
                            Write-WWLog -data "Unable to download update item content. Error message: $($_.Exception.Message)" -Class Error
                            $ReturnValue = 1
                        }
                    } else {
                        Write-WWLog -data "Unable to determine update content instance for CI_ID: $($UpdateItemContentID.ContentID)" -Class Error
                        $ReturnValue = 1
                    }
                }
            } else {
                Write-WWLog -Data "Unable to determine ContentID instance for CI_ID: $($UpdateItem.CI_ID)" -Class Error
                $ReturnValue = 1
            }
        } else {
            Write-WWLog -data "Unable to locate update item from SMS Provider for update type: $($UpdateType)" -Class Error
            $ReturnValue = 2
        }
